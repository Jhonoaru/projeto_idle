...
const discountedGuild = { ...guild, headquarters: { facilityLevels: { quartermaster: 3 } } };
const discountedPurchase = buyFromNpcShop(hero, discountedGuild, emptyDepot, 'small-backpack', 2, 118, 'guild_depot');
assert.equal(discountedPurchase.success, true, 'NPC accepts displayed 6% discount');
assert.equal(discountedPurchase.guild.gold, guild.gold - 236);
assert.equal(buyFromNpcShop(hero, discountedGuild, emptyDepot, 'small-backpack', 1, 1, 'guild_depot').success, false);
assert.equal(buyFromNpcShop(hero, discountedGuild, emptyDepot, 'small-backpack', 1, 125, 'guild_depot').success, false);
