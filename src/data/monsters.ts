import type { Monster } from "../shared/types";

export const monsters = {
  sewerRat: {
    id: "monster-sewer-rat",
    name: "Sewer Rat",
    level: 1,
    health: 22,
    experience: 42,
    minDamage: 1,
    maxDamage: 4,
    armor: 0,
    defense: 1,
    resistances: { fire: -10 },
    conditionResistances: { burn: -10, poison: 0, slow: 0 },
    evasionPercent: 2,
    goldMin: 1,
    goldMax: 5,
    lootTable: [
      { itemId: "gold-coin", chance: 0.55, minQuantity: 1, maxQuantity: 3 },
      { itemId: "rat-tail", chance: 1, minQuantity: 1, maxQuantity: 2 },
      { itemId: "minor-health-potion", chance: 0.025, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  caveSpider: {
    id: "monster-cave-spider",
    name: "Cave Spider",
    level: 4,
    health: 48,
    experience: 54,
    minDamage: 3,
    maxDamage: 10,
    armor: 1,
    defense: 3,
    resistances: { earth: 15, fire: -15 },
    conditionResistances: { burn: -15, slow: 5 },
    conditionImmunities: ["poison"],
    conditionAttacks: [{ type: "poison", applicationChancePercent: 22, durationSeconds: 6, tickIntervalSeconds: 2, damagePercentPerTick: 2 }],
    evasionPercent: 8,
    goldMin: 1,
    goldMax: 7,
    lootTable: [
      { itemId: "gold-coin", chance: 0.48, minQuantity: 1, maxQuantity: 6 },
      { itemId: "spider-silk", chance: 0.18, minQuantity: 1, maxQuantity: 2 },
      { itemId: "broken-fang", chance: 0.05, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  forestTroll: {
    id: "monster-forest-troll",
    name: "Forest Troll",
    level: 12,
    health: 130,
    experience: 85,
    minDamage: 8,
    maxDamage: 24,
    armor: 4,
    defense: 8,
    resistances: { earth: 10, fire: -20 },
    conditionResistances: { burn: -20, poison: 10, slow: 5 },
    conditionAttacks: [{ type: "poison", applicationChancePercent: 12, durationSeconds: 6, tickIntervalSeconds: 3, damagePercentPerTick: 2 }],
    evasionPercent: 4,
    goldMin: 8,
    goldMax: 22,
    lootTable: [
      { itemId: "gold-coin", chance: 0.7, minQuantity: 6, maxQuantity: 18 },
      { itemId: "troll-tooth", chance: 0.2, minQuantity: 1, maxQuantity: 2 },
      { itemId: "old-cloth", chance: 0.12, minQuantity: 1, maxQuantity: 3 },
      { itemId: "iron-longsword", chance: 0.008, minQuantity: 1, maxQuantity: 1 },
      { itemId: "iron-handwraps", chance: 0.008, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  mudRotter: {
    id: "monster-mud-rotter",
    name: "Mud Rotter",
    level: 18,
    health: 190,
    experience: 145,
    minDamage: 12,
    maxDamage: 34,
    armor: 7,
    defense: 12,
    resistances: { earth: 20, holy: -15 },
    conditionResistances: { burn: 10, poison: 60, slow: 15 },
    conditionAttacks: [{ type: "poison", applicationChancePercent: 28, durationSeconds: 9, tickIntervalSeconds: 3, damagePercentPerTick: 3 }],
    evasionPercent: 6,
    goldMin: 12,
    goldMax: 32,
    lootTable: [
      { itemId: "gold-coin", chance: 0.72, minQuantity: 10, maxQuantity: 28 },
      { itemId: "rotten-claw", chance: 0.18, minQuantity: 1, maxQuantity: 2 },
      { itemId: "health-potion", chance: 0.04, minQuantity: 1, maxQuantity: 1 },
      { itemId: "runed-wand", chance: 0.006, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  youngMinotaur: {
    id: "monster-young-minotaur",
    name: "Young Minotaur",
    level: 27,
    health: 310,
    experience: 260,
    minDamage: 20,
    maxDamage: 52,
    armor: 12,
    defense: 18,
    resistances: { physical: 10, ice: -10 },
    conditionResistances: { burn: 0, poison: 5, slow: 10 },
    evasionPercent: 5,
    goldMin: 22,
    goldMax: 58,
    lootTable: [
      { itemId: "gold-coin", chance: 0.82, minQuantity: 18, maxQuantity: 50 },
      { itemId: "minotaur-horn", chance: 0.16, minQuantity: 1, maxQuantity: 2 },
      { itemId: "brass-shield", chance: 0.02, minQuantity: 1, maxQuantity: 1 },
      { itemId: "iron-cuirass", chance: 0.01, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  orcRaider: {
    id: "monster-orc-raider",
    name: "Orc Raider",
    level: 25,
    health: 280,
    experience: 230,
    minDamage: 18,
    maxDamage: 48,
    armor: 10,
    defense: 16,
    resistances: { physical: 5, energy: -10 },
    conditionResistances: { burn: 5, poison: 10, slow: 10 },
    evasionPercent: 10,
    goldMin: 18,
    goldMax: 52,
    lootTable: [
      { itemId: "gold-coin", chance: 0.78, minQuantity: 14, maxQuantity: 44 },
      { itemId: "orc-leather", chance: 0.18, minQuantity: 1, maxQuantity: 2 },
      { itemId: "training-axe", chance: 0.025, minQuantity: 1, maxQuantity: 1 },
      { itemId: "ironwood-bow", chance: 0.008, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  dwarfGuard: {
    id: "monster-dwarf-guard",
    name: "Dwarf Guard",
    level: 42,
    health: 580,
    experience: 560,
    minDamage: 38,
    maxDamage: 92,
    armor: 24,
    defense: 34,
    resistances: { physical: 20, fire: 10, ice: -10 },
    conditionResistances: { burn: 20, poison: 15, slow: 20 },
    conditionAttacks: [{ type: "slow", applicationChancePercent: 18, durationSeconds: 5, potencyPercent: 12 }],
    evasionPercent: 3,
    goldMin: 45,
    goldMax: 110,
    lootTable: [
      { itemId: "gold-coin", chance: 0.88, minQuantity: 40, maxQuantity: 100 },
      { itemId: "dwarf-badge", chance: 0.12, minQuantity: 1, maxQuantity: 1 },
      { itemId: "iron-ore", chance: 0.22, minQuantity: 1, maxQuantity: 4 },
      { itemId: "gravewood-bow", chance: 0.005, minQuantity: 1, maxQuantity: 1 },
      { itemId: "cryptguard-armor", chance: 0.004, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  ancientSkeleton: {
    id: "monster-ancient-skeleton",
    name: "Ancient Skeleton",
    level: 34,
    health: 430,
    experience: 410,
    minDamage: 28,
    maxDamage: 72,
    armor: 18,
    defense: 24,
    resistances: { physical: 10, death: 25, holy: -25 },
    conditionResistances: { burn: 15, slow: 30 },
    conditionImmunities: ["poison"],
    evasionPercent: 7,
    goldMin: 28,
    goldMax: 78,
    lootTable: [
      { itemId: "gold-coin", chance: 0.76, minQuantity: 24, maxQuantity: 70 },
      { itemId: "ancient-bone", chance: 0.18, minQuantity: 1, maxQuantity: 3 },
      { itemId: "enchanted-dust", chance: 0.035, minQuantity: 1, maxQuantity: 1 },
      { itemId: "cryptsteel-blade", chance: 0.005, minQuantity: 1, maxQuantity: 1 },
      { itemId: "boneweave-wraps", chance: 0.005, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  cyclopsBrute: {
    id: "monster-cyclops-brute",
    name: "Cyclops Brute",
    level: 48,
    health: 760,
    experience: 760,
    minDamage: 48,
    maxDamage: 122,
    armor: 28,
    defense: 42,
    resistances: { physical: 15, energy: -15 },
    conditionResistances: { burn: 5, poison: 15, slow: 25 },
    evasionPercent: 2,
    goldMin: 60,
    goldMax: 150,
    lootTable: [
      { itemId: "gold-coin", chance: 0.9, minQuantity: 55, maxQuantity: 140 },
      { itemId: "cyclops-eye", chance: 0.035, minQuantity: 1, maxQuantity: 1 },
      { itemId: "wooden-club", chance: 0.03, minQuantity: 1, maxQuantity: 1 },
      { itemId: "cryptguard-armor", chance: 0.006, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  wyvernHatchling: {
    id: "monster-wyvern-hatchling",
    name: "Wyvern Hatchling",
    level: 55,
    health: 860,
    experience: 980,
    minDamage: 55,
    maxDamage: 145,
    armor: 24,
    defense: 38,
    resistances: { fire: 15, earth: 10, ice: -15 },
    conditionResistances: { burn: 25, poison: 20, slow: -10 },
    conditionAttacks: [{ type: "poison", applicationChancePercent: 25, durationSeconds: 9, tickIntervalSeconds: 3, damagePercentPerTick: 3 }],
    evasionPercent: 14,
    goldMin: 75,
    goldMax: 180,
    lootTable: [
      { itemId: "gold-coin", chance: 0.92, minQuantity: 70, maxQuantity: 170 },
      { itemId: "wyvern-scale", chance: 0.14, minQuantity: 1, maxQuantity: 2 },
      { itemId: "broken-fang", chance: 0.16, minQuantity: 1, maxQuantity: 3 },
      { itemId: "wyvern-bow", chance: 0.003, minQuantity: 1, maxQuantity: 1 },
      { itemId: "dragon-wraps", chance: 0.003, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  dragonWhelp: {
    id: "monster-dragon-whelp",
    name: "Dragon Whelp",
    level: 62,
    health: 1120,
    experience: 1350,
    minDamage: 70,
    maxDamage: 180,
    armor: 32,
    defense: 46,
    resistances: { fire: 25, physical: 10, ice: -25 },
    conditionResistances: { poison: 35, slow: -25 },
    conditionImmunities: ["burn"],
    conditionAttacks: [{ type: "burn", applicationChancePercent: 30, durationSeconds: 8, tickIntervalSeconds: 2, damagePercentPerTick: 3 }],
    evasionPercent: 10,
    goldMin: 105,
    goldMax: 260,
    lootTable: [
      { itemId: "gold-coin", chance: 0.95, minQuantity: 90, maxQuantity: 240 },
      { itemId: "dragon-ember", chance: 0.025, minQuantity: 1, maxQuantity: 1 },
      { itemId: "wyvern-scale", chance: 0.18, minQuantity: 1, maxQuantity: 2 },
      { itemId: "strong-health-potion", chance: 0.05, minQuantity: 1, maxQuantity: 1 },
      { itemId: "ember-blade", chance: 0.002, minQuantity: 1, maxQuantity: 1 },
      { itemId: "ember-staff", chance: 0.002, minQuantity: 1, maxQuantity: 1 },
      { itemId: "dragonscale-armor", chance: 0.0015, minQuantity: 1, maxQuantity: 1 },
    ],
  },
  cultAcolyte: {
    id: "monster-cult-acolyte",
    name: "Cult Acolyte",
    level: 36,
    health: 390,
    experience: 450,
    minDamage: 32,
    maxDamage: 82,
    armor: 12,
    defense: 20,
    resistances: { death: 20, holy: -20, energy: 10 },
    conditionResistances: { burn: 5, poison: 25, slow: 15 },
    conditionAttacks: [{ type: "slow", applicationChancePercent: 20, durationSeconds: 6, potencyPercent: 15 }],
    evasionPercent: 12,
    goldMin: 40,
    goldMax: 95,
    lootTable: [
      { itemId: "gold-coin", chance: 0.82, minQuantity: 35, maxQuantity: 90 },
      { itemId: "cultist-charm", chance: 0.1, minQuantity: 1, maxQuantity: 1 },
      { itemId: "mana-potion", chance: 0.08, minQuantity: 1, maxQuantity: 2 },
      { itemId: "crypt-scepter", chance: 0.006, minQuantity: 1, maxQuantity: 1 },
    ],
  },
} satisfies Record<string, Monster>;
